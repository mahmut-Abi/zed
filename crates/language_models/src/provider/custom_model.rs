...
use custom_model::{ImageUrl, Model, ResponseStreamEvent, stream_completion};
use fs::Fs;
use futures::Stream;
...
use crate::CustomSettingsContent;
use crate::{AllLanguageModelSettings, ui::InstructionListItem};

const PROVIDER_ID: LanguageModelProviderId = language_model::CUSTOM_PROVIDER_ID;
const PROVIDER_NAME: LanguageModelProviderName = language_model::CUSTOM_PROVIDER_NAME;
...
const CUSTOM_API_KEY_VAR: &str = "CUSTOM_API_KEY";
...
impl State {
    ...
    fn authenticate(&self, cx: &mut Context<Self>) -> Task<Result<(), AuthenticateError>> {
        ...
        let api_url = AllLanguageModelSettings::get_global(cx)
            ...
            .clone();
        cx.spawn(async move |this, cx| {
            let (api_key, from_env) = if let Ok(api_key) = std::env::var(CUSTOM_API_KEY_VAR) {
                (api_key, true)
            } else {
...
impl CustomLanguageModel {
    ...
    ) -> BoxFuture<'static, Result<futures::stream::BoxStream<'static, Result<ResponseStreamEvent>>>>
    {
        ...
        let Ok((api_key, api_url)) = cx.read_entity(&self.state, |state, cx| {
            let settings = &AllLanguageModelSettings::get_global(cx).custom;
...
impl LanguageModel for CustomLanguageModel {
    ...
    > {
        ...
        async move {
            let mapper = CustomEventMapper::new();
...
pub struct CustomEventMapper {
    tool_calls_by_index: HashMap<usize, RawToolCall>,
}

impl CustomEventMapper {
    pub fn new() -> Self {
        Self {
    ...
    pub fn map_event(
        ...
        match choice.finish_reason.as_deref() {
            ...
            Some(stop_reason) => {
                log::error!("Unexpected Custom stop_reason: {stop_reason:?}",);
                events.push(Ok(LanguageModelCompletionEvent::Stop(StopReason::EndTurn)));
            }
            None => {}
        }
    ...
    fn save_api_url(&mut self, cx: &mut Context<Self>) {
        ...
        if !api_url.is_empty() && api_url != effective_current_url {
            ...
            update_settings_file::<AllLanguageModelSettings>(fs, cx, move |settings, _| {
                if let Some(settings) = settings.custom.as_mut() {
                    settings.api_url = Some(api_url.clone());
                } else {
                    settings.custom = Some(CustomSettingsContent {
    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new("To use Zed's assistant with Custom, you need to add an API key. Follow these steps:"))
                .child(
                    List::new()
                        .child(InstructionListItem::new(
                            "Create one by visiting",
                            Some("Custom's console"),
                            Some("https://xxx.xxx.com/api-keys"),
                        ))
                        .child(InstructionListItem::text_only(
                            "Ensure your Custom account has credits",
                        ))
                        .child(InstructionListItem::text_only(
                ...
                .child(
                    Label::new(
                        format!("You can also assign the {CUSTOM_API_KEY_VAR} environment variable and restart Zed."),
                    )
                ...
                .child(
                    h_flex()
                        ...
                        .child(Label::new(if env_var_set {
                            format!("API key set in {CUSTOM_API_KEY_VAR} environment variable.")
                        } else {
                ...
                .child(
                    Button::new("reset-api-key", "Reset API Key")
                        ...
                        .when(env_var_set, |this| {
                            this.tooltip(Tooltip::text(format!("To reset your API key, unset the {CUSTOM_API_KEY_VAR} environment variable.")))
                        })
        ...
        let api_url_section = if custom_api_url_set {
            ...
            v_flex()
                ...
                .child(
                    List::new()
                        .child(InstructionListItem::text_only(
                            "Optionally, you can change the base URL for the Custom API request.",
                        ))
                        .child(InstructionListItem::text_only(
