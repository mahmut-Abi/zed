...
pub const CUSTOM_API_URL: &str = "https://xxx.xxx.com/v1";
...
pub async fn stream_completion(
    ...
    if response.status().is_success() {
        ...
        match serde_json::from_str::<CustomResponse>(&body) {
...
#[derive(Serialize)]
struct CustomEmbeddingRequest<'a> {
    model: CustomEmbeddingModel,
    input: Vec<&'a str>,
}

#[derive(Deserialize)]
pub struct CustomEmbeddingResponse {
    pub data: Vec<CustomEmbedding>,
}
...
pub fn embed<'a>(
...
) -> impl 'static + Future<Output = Result<CustomEmbeddingResponse>> {
    ...
    async move {
        ...
        let response: CustomEmbeddingResponse =
            serde_json::from_str(&body).context("failed to parse Custom embedding response")?;
        Ok(response)
    }
}
